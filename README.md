...
- **Tests.** htop ships no test suite (`make check` is automake's empty no-op — the only `check`-adjacent target is a `cppcheck` lint, not a runtime harness), so `doCheck` has nothing to run. `htop -V` is the smoke floor.
